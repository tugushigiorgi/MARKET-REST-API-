            ...
            CreateMap<Receipt, ReceiptModel>()
                ...
                .ReverseMap();
                 ...
                 CreateMap<Product, ProductModel>()
                     .ForMember(dest => dest.ProductName, opt => opt.MapFrom(d=>d.ProductName)) 

                .ForMember(dest => dest.CategoryName, opt => opt.MapFrom(src => src.Category.CategoryName))
                .ForMember(dest => dest.ReceiptDetailIds, opt => opt.MapFrom(src => src.ReceiptDetails.Select(rd => rd.Id))) 
                .ForMember(dest => dest.ProductCategoryId, opt => opt.MapFrom(d=>d.ProductCategoryId)); // Ignore ReceiptDetails


                 CreateMap<ProductModel, Product>()
                     .ForMember(dest => dest.Category,
                         opt => opt.MapFrom(src => new ProductCategory
                             { CategoryName = src.CategoryName  })) // Map Category from CategoryName
                     .ForMember(dest => dest.ReceiptDetails, opt => opt.MapFrom(d=>d.ReceiptDetailIds))
                     .ForMember(dest => dest.ProductName, opt => opt.MapFrom(d => d.ProductName))
                     .ForMember(dest => dest.Id, opt => opt.MapFrom(d => d.Id))
                     .ForMember(dest => dest.ProductCategoryId, opt => opt.MapFrom(d => d.ProductCategoryId));
